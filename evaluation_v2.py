...
def calculate_map(predictions, ground_truths, iou_threshold=0.5):
    aps = []
    for class_id in ground_truths.keys():
        all_detected_boxes = predictions[class_id]
        all_gt_boxes = ground_truths[class_id]
        ap = average_precision(all_detected_boxes, all_gt_boxes, iou_threshold)
        aps.append(ap)
    ...
    for example in data:
        ...
        if question_id == 0:
            ...
            if model_data["Bbox"] != "Not Found" and true_data["Bbox"] != "Not Found":
                ...
                confidences = 1.0
                #model_class in 0 or 1
                if model_data["Class"] in [0, 1]:
                    predictions_bbox[model_data["Class"]].append((model_data["Bbox"], confidences))
                if true_data["Class"] in [0, 1]:
                    ground_truth_bbox[true_data["Class"]].append(true_data["Bbox"])
